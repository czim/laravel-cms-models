<?php
namespace Czim\CmsModels\Test\Helpers\Traits;
...
trait InteractsWithPages
{
    ...
    protected function makeRequest($method, $uri, $parameters = [], $cookies = [], $files = [])
    {
        ...
        $response = $this->clearInputs()->followRedirects($response);
        $response = $this->assertPageLoaded($response, $uri);

        $this->currentUri = $this->app->make('request')->fullUrl();

        $this->crawler = new Crawler($response->getContent(), $this->currentUri);
    ...
    protected function followRedirects($response)
    {
        while ($response->isRedirect()) {
            $response = $this->makeRequest('GET', $response->headers->get('location'));
        }
     ...
     */
    protected function assertPageLoaded($response, $uri, $message = null)
    {
        ...
        try {
            ...
            $responseException = isset($response->exception)
